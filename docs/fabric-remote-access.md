...
| **5 — Native client + hardening** ◑ **PARTIAL** | ✅ `cerebro` CLI (`cli/`, Go): `ls` + `access <machine> [ssh\|rdp]` opens a local listener and tunnels it over `/api/fabric/access/ws` (bearer API token, `fabric:read`+`fabric:connect` now grantable); built + served at `/api/fabric/cli/binary`; **Command line** dialog in the UI. ✅ offline reconciler + alert (polish pass). *Remaining:* session recording, approval gate. | Power-user path (own ssh/scp/mstsc); enterprise controls next. |
